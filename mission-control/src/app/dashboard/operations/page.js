...
function Metric({ label, value }) {
  return (
    <div className="rounded-lg border border-line bg-black/25 p-4">
      <dt className="label-mono text-white/70">{label}</dt>
    ...
    <div className="mx-auto max-w-6xl space-y-7">
      ...
      <section aria-labelledby="sample-data-title" className="rounded-xl border border-line bg-surface p-6">
        ...
        {!sampleStatusError && <>
          ...
          <details className="mt-5 rounded-lg border border-line bg-black/25 p-4">
            ...
            <ul className="mt-4 grid gap-2 font-mono text-xs text-white/70 sm:grid-cols-2">
      ...
      <section aria-labelledby="sample-child-space-title" className="rounded-xl border border-line bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="label-mono text-gold">Hierarchy hygiene</p><h2 id="sample-child-space-title" className="mt-2 text-lg font-semibold">Sample child-space cleanup</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/70">Reviews Units_JSON only for the fixed seven-property sample allowlist and removes empty or unmistakably placeholder child rows. Authored inventory remains untouched.</p></div>
        ...
        {childSpaceStatus && <>
          ...
          {childSpaceStatus.invalid.length > 0 && <div className="mb-6 overflow-x-auto rounded-lg border border-line"><table className="w-full min-w-[680px] text-left text-xs"><thead className="bg-black/35 text-white/70"><tr><th className="p-3">Property</th><th className="p-3">Current child name</th><th className="p-3">Reason</th><th className="p-3">Child ID</th></tr></thead><tbody className="divide-y divide-line">{childSpaceStatus.invalid.map((entry) => <tr key={`${entry.recordId}:${entry.index}`} className="bg-danger/5 text-danger"><td className="p-3"><span className="block text-white/80">{entry.title}</span><span className="font-mono text-[12px]">{entry.slug}</span></td><td className="p-3">{entry.name || "(empty)"}</td><td className="p-3 font-mono">{entry.reason}</td><td className="p-3 font-mono">{entry.unitId || "none"}</td></tr>)}</tbody></table></div>}
      ...
      <section aria-labelledby="media-review-title" className="rounded-xl border border-line bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/70">Reviews Video, Matterport, Luma, and drone heatmap fields with the same provider rules used by the public renderer.</p>
          </div>
        ...
        {mediaStatus && <>
          ...
          <div className="mb-6 overflow-x-auto rounded-lg border border-line">
            <table className="w-full min-w-[760px] text-left text-xs">
              <thead className="bg-black/35 text-white/70"><tr><th className="p-3">Property</th><th className="p-3">Field</th><th className="p-3">Classification</th><th className="p-3">Current value</th></tr></thead>
              <tbody className="divide-y divide-line">{mediaStatus.entries.map((entry) => <tr key={`${entry.recordId}:${entry.field}`} className={entry.safe ? "text-white/70" : "bg-danger/5 text-danger"}>
                <td className="p-3"><span className="block text-white/80">{entry.title}</span><span className="font-mono text-[12px]">{entry.slug || entry.recordId}</span></td>
      ...
      <section aria-labelledby="lifecycle-review-title" className="rounded-xl border border-line bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="label-mono text-gold">Dual-CMS trust</p><h2 id="lifecycle-review-title" className="mt-2 text-lg font-semibold">Airtable / Supabase public lifecycle reconciliation</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-white/70">Compares every Airtable-public slug with its canonical Supabase lifecycle and server-resolved inquiry recipient. Mission Control never invents a missing property or recipient.</p></div>
      ...
      <section aria-labelledby="pilot-cohort-title" className="rounded-xl border border-line bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 font-mono text-xs text-white/70">{PILOT_COHORT_MIGRATION.id}</p>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-white/70">Creates two service-role-only tables for cohort windows, verified tester user IDs, roles, offboarding, and account-deletion evidence. It stores no raw temporary email and adds no pilot flags to product tables.</p>
          </div>
        ...
        {!pilotCohortStatusError && <>
          ...
          <details className="mt-5 rounded-lg border border-line bg-black/25 p-4"><summary className="cursor-pointer text-sm font-medium text-white/75">Review recovery evidence</summary><pre className="mt-4 whitespace-pre-wrap text-xs leading-5 text-white/70">{PILOT_COHORT_ROLLBACK_PLAN}</pre></details>
      ...
      <section aria-labelledby="readiness-title" className="rounded-xl border border-line bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 font-mono text-xs text-white/70">{ONBOARDING_MIGRATION.id}</p>
          </div>
      ...
      <section aria-labelledby="wishlist-revocation-title" className="rounded-xl border border-line bg-surface p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="label-mono text-gold">Privacy control</p><h2 id="wishlist-revocation-title" className="mt-2 text-lg font-semibold">Wishlist share-link revocation</h2>
            <p className="mt-1 font-mono text-xs text-white/70">{WISHLIST_REVOCATION_MIGRATION.id}</p>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-white/70">Adds a service-role-only revocation watermark so a member can deactivate every previously issued public Board link without storing bearer tokens.</p></div>
        ...
        {!wishlistStatusError && <>
          ...
          <details className="mt-5 rounded-lg border border-line bg-black/25 p-4"><summary className="cursor-pointer text-sm font-medium text-white/75">Review recovery evidence</summary><pre className="mt-4 whitespace-pre-wrap text-xs leading-5 text-white/70">{WISHLIST_REVOCATION_ROLLBACK_PLAN}</pre></details>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section aria-labelledby="impact-title" className="rounded-xl border border-line bg-surface p-6">
          ...
          <p className="mt-4 text-xs leading-5 text-white/70">Adds three nullable columns, restores the one-role contract, and backfills only derivable, age-eligible accounts.</p>
        </section>
        ...
        <section aria-labelledby="recovery-title" className="rounded-xl border border-line bg-surface p-6">
          ...
          <dl className="mt-5 space-y-3 text-sm">
            <div className="flex justify-between gap-4 border-b border-line pb-3"><dt className="text-white/70">Latest recovery point</dt><dd className="text-right text-white/80">{status?.backup?.latestBackup?.inserted_at || status?.backup?.pitrAt || "Unavailable"}</dd></div>
            <div className="flex justify-between gap-4 border-b border-line pb-3"><dt className="text-white/70">PITR</dt><dd>{status?.backup?.pitrEnabled ? "Enabled" : "Not reported"}</dd></div>
            <div className="flex justify-between gap-4"><dt className="text-white/70">Private fields in public views</dt><dd>{status?.privacySafe === true ? "None detected" : status?.privacySafe === false ? "Exposure detected" : "Not checked"}</dd></div>
          </dl>
          <details className="mt-5 rounded-lg border border-line bg-black/25 p-4">
            ...
            <pre className="mt-4 overflow-x-auto whitespace-pre-wrap text-xs leading-5 text-white/70">{ONBOARDING_ROLLBACK_PLAN}</pre>
          </details>
        </section>
      </div>
      ...
      <section aria-labelledby="execution-title" className="rounded-xl border border-gold-muted/50 bg-surface p-6">
        ...
        <details className="mt-5 border-t border-line pt-4">
          <summary className="cursor-pointer text-xs font-medium text-white/70 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold">Review immutable checksum</summary>
        ...
        <div className="mt-4 divide-y divide-line">
          {(history ?? []).length ? history.map((event) => (
            <div key={event.id} className="grid gap-1 py-3 text-sm sm:grid-cols-[13rem_1fr_auto] sm:gap-4">
              <span className="font-mono text-xs text-gold">{event.action}</span><span className="text-white/70">{event.reason || "No reason recorded"}</span><time className="text-xs text-white/70">{new Date(event.created_at).toLocaleString()}</time>
            </div>
          )) : <p className="py-4 text-sm text-white/70">No attempts recorded.</p>}
        </div>
      </section>
    </div>
  );
