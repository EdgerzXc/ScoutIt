// ⚠️ VENDORED from the main app's src/lib/intelPublish.js on 2026-08-30.
//
// Mission Control is a separate deployment and cannot import from the main
// app, so the Airtable INTEL_CMS mapping is duplicated here — the same way
// propertyFieldMapping.js already is. A test asserts the two copies stay in
// step on the field mapping, because a silent divergence would publish
// articles with different shapes depending on which app pressed the button.
//
// One deliberate difference: the main app defaults fetchImpl to its bounded
// fetchWithRetry. This app has no such helper, so the default is plain fetch.
// The injectable seam is unchanged.
...
export function buildIntelFields(briefing, relatedPropertyIds = []) {
  ...
  // byte-identical in the main app's copy — the parity test pins it.
