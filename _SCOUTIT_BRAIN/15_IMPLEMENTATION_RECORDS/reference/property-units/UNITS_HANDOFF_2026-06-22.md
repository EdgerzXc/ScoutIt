---
section: "15_IMPLEMENTATION_RECORDS/reference/property-units"
status: reference
tags: [units, handoff, data-layer, session-log, resolved]
updated: 2026-07-30
related: ["[[DATA_DICTIONARY]]", "[[WEBSITE_ARCHITECTURE]]", "[[15_IMPLEMENTATION_RECORDS/README|Implementation Records]]", "[[AIRTABLE_COMPRESSION_PLAN]]"]
---

> ⚠️ This is a historical, dated handoff (2026-06-22, last touched 2026-06-29) — kept for record,
> not current truth. For the actual latest build status, read [[15_IMPLEMENTATION_RECORDS/README|Implementation Records]] first.

> ## ✅ 2026-07-30 — THE UNITS STORAGE QUESTION IS CLOSED
>
> Anything below that treats "units as first-class Airtable rows" as the
> direction is **superseded**. The owner ruled: **one row per building. Units
> stay inside `Units_JSON`.** The empty `UNITS` Airtable table
> (`tblfvXBgDzY1l9OpJ`, 0 records) was **deleted**.
>
> **The reasoning, so nobody rebuilds it:** Airtable caps on **records (rows)**,
> not columns, and the free plan's **1,000-record limit is cumulative across the
> whole base**. Units-as-rows makes 100 units = 100 records, so a handful of
> large towers exhausts the free tier and forces the Team plan (~$20/seat/mo).
> `Units_JSON` makes 100 units = **1 record**, so the 200-listing goal fits free.
>
> An intermediate idea — numbered columns (`Unit1_Name`, `Unit2_Name`…) with
> overflow rows for big buildings — was considered and also rejected. Airtable
> caps at 500 columns per table (186 used), and because `Slug` is a **formula
> computed from Title**, two rows for one building would produce the *same slug*:
> the building would list twice publicly, and `updateProperty`'s `maxRecords=1`
> lookup would silently edit only the first row.
>
> `Units_JSON` already does what that design was reaching for, with **no ceiling**.
>
> **What would reopen it:** genuine cross-building unit search. Nothing else.
> Full write-up: [[AIRTABLE_COMPRESSION_PLAN]] §5 · schema record:
> [[DATA_DICTIONARY]] §3.2.
>
> ℹ️ Supabase `property_units` is **unaffected** and remains the authoritative
> unit store. This ruling only concerns the Airtable public mirror.
