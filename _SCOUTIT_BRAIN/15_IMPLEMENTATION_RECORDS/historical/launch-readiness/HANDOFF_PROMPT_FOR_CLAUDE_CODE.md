...
status: stale
tags: [implementation-record, historical, launch-readiness, handoff]
updated: 2026-08-02
related: ["[[15_IMPLEMENTATION_RECORDS/README|Implementation Records]]", "[[PRODUCTION_READINESS]]", "[[SCOUTIT_FIX_LIST]]"]
---

# Handoff Prompt for Claude Code — ScoutIt Human-Testing Readiness

> **Historical record (2026-07-22).** Do not run this as the current launch plan. Start with
> [[00_MASTER_SYNC]], [[SCOUTIT_FIX_LIST]], and runtime code.
...
platform deployed on Vercel (`scout-it.vercel.app`). Read the repository-root `AGENTS.md` and
this record's [[PRODUCTION_READINESS]] snapshot first for its original context. Also read
`mission-control/MISSION_CONTROL_SPEC.md` for the staff-console context. Per `AGENTS.md`, this
Next.js version has breaking changes — read the relevant
...
- Docs added: [[PRODUCTION_READINESS]], [[MASTER_MISSION_CONTROL_HARDENING]].
...
Once `auth.uid()` is real, review `supabase/operations/security/supabase_rls_hardening.sql` so `owner_id = auth.uid()`
...
## When done
Update [[PRODUCTION_READINESS]] to move completed items to the Done section, run the Supabase
