...
status: stale
tags: [implementation-record, historical, coordination, agent-queue]
updated: 2026-08-02
related: ["[[15_IMPLEMENTATION_RECORDS/README|Implementation Records]]", "[[00_MASTER_SYNC]]", "[[CLAUDE_CODE_MASTER_BRIEF]]"]
---

# ScoutIt — Work Split: Claude Code vs Cowork (Claude)

> **Historical record (2026-07-23).** The prompt index below documents an earlier agent split.
> Use [[00_MASTER_SYNC]] and the current codebase for today's queue.
...
1. **Run local dev + verify / debug "can't click."** Confirm every sidebar route loads for the
   ...
   non-interactive (not a bug). → [[HANDOFF_PROMPT_RUN_MISSION_CONTROL_LOCAL]]
2. **Visual redesign + instant navigation.** Gold-on-black ScoutIt DNA, dense/scannable; add
   ...
   sidebar; define the gold token system in `globals.css`. → [[HANDOFF_PROMPT_REDESIGN_MISSION_CONTROL]]
3. **Metrics redesign** — real bar charts + Daily/Weekly/Monthly toggle. → [[HANDOFF_PROMPT_METRICS_AND_IP_GUARD]] (Feature 1)
4. **Masked IP Anomaly Guard — Phase 1 (safe):** the Mission Control "Security" HUD (flagged hashed
   IPs + 1-click block/unblock via the existing `blocked_access` list). → [[HANDOFF_PROMPT_METRICS_AND_IP_GUARD]] (Feature 2, Phase 1)
...
- [[HANDOFF_PROMPT_RUN_MISSION_CONTROL_LOCAL]] — run local dev + verify/debug.
- [[HANDOFF_PROMPT_REDESIGN_MISSION_CONTROL]] — visual redesign + instant navigation.
- [[HANDOFF_PROMPT_METRICS_AND_IP_GUARD]] — metrics bar charts + Masked IP Anomaly Guard (phased).
- [[HANDOFF_PROMPT_DEPLOY_MISSION_CONTROL]] — deploy to Vercel (already used).
- [[HANDOFF_PROMPT_FOR_CLAUDE_CODE]] — earlier main-site readiness (auth, ledger merge, CI).
- [[PRODUCTION_READINESS]], [[MASTER_MISSION_CONTROL_HARDENING]] — reference plans.
