...
//
// Pure render engine for the Golden Horizon raymarched black hole. No React,
// no HUD, no DOM chrome, no interaction policy of its own: it draws what its
// callbacks and params tell it to draw.
//
// WHAT MOVED HERE (verbatim from GoldenHorizonCanvas.js, behavior unchanged):
//   · GOLDEN_PALETTE_NAMES / GOLDEN_PRESETS / GOLDEN_DEFAULT_PARAMS
//   · VERTEX_SHADER_SRC / FRAGMENT_SHADER_SRC (fragment copied verbatim from
//     the generator's BlackHoleCanvas.tsx — do not "improve" the math)
//   · GL setup (compile/link/quad buffer/uniform lookup), DPR-1 resize,
//     the render loop with orbit momentum + auto-spin drift, click ripple,
//     hover lensing, hold-to-dolly easing, and every guard: reduced-motion
//     single frame, IntersectionObserver pause, visibilitychange pause,
//     Lite-Mode live teardown, GL resource cleanup (no forced context loss —
//     Strict Mode's dev double-mount reuses the canvas).
//
// WHAT STAYED IN THE CONSUMER (GoldenHorizonCanvas.js):
//   · React state (revealed, isCapturing3D), paramsRef syncing, HUD JSX/CSS,
//     pointer-listener attachment. The consumer passes preserveDrawingBuffer:
//     true because the UFO easter egg captures 3D — that cost is opt-in here
//     (default false) so the future cinematic never pays for it.
//
// Phase-0 exit test lives in the consumer: identical pixels, identical
// behavior, no assertion re-aimed anywhere.
// ═══════════════════════════════════════════════════════════════
  ...
  float noise(in vec3 x) {
    ...
    return mix(
      ...
      mix(
        ...
        mix(hash(p + vec3(0,1,1)), hash(p + vec3(1,0,1)), f.x),
        f.y
      ),
      f.z
    );
...
/**
 * Create a black-hole renderer on a canvas. Framework-free: no React, no HUD,
 * no DOM chrome. Returns an imperative handle the consumer wires to its own
 * interaction layer and lifecycle.
 *
 * @param {HTMLCanvasElement} canvas
 * @param {object} opts
 * @param {boolean} [opts.preserveDrawingBuffer=false] — opt-in only. Needed
 *   for snapshot capture (the UFO easter egg); a real cost the cinematic
 *   does not need.
 * @param {() => object} opts.getParams — live params object (the consumer's
 *   paramsRef.current). Read per frame, so a `params` prop change needs no
 *   re-init.
 * @param {{pitch:number,yaw:number}} opts.initialCamera — camera snapshot.
 * @param {() => boolean} opts.isLiteMode
 * @param {string} opts.liteModeEvent — event name toggling Lite Mode live.
 * @param {() => void} [opts.onFirstFrame] — fired after the first painted frame.
 * @param {(capturing:boolean) => void} [opts.onCaptureChange]
 * @param {(hud:{pitch:number,yaw:number,zoomLabel:string}) => void} [opts.onHud]
 * @returns {{beginDrag,endDrag,moveDrag,destroy}|null} null when Lite Mode is
 *   on at setup or WebGL is unavailable (consumer renders its fallback).
 */
export function createRenderer(canvas, opts = {}) {
  const {
    preserveDrawingBuffer = false,
    getParams,
    initialCamera = { pitch: 0, yaw: 0 },
    isLiteMode = () => false,
    liteModeEvent = "scoutit:litemode",
    onFirstFrame = () => {},
    onCaptureChange = () => {},
    onHud = () => {},
  } = opts;
  ...
  const compileShader = (src, type) => {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      console.error("[BlackHoleEngine] shader compile failed:", gl.getShaderInfoLog(shader));
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error("[BlackHoleEngine] program link failed:", gl.getProgramInfoLog(program));
    return null;
  }
  ...
  // ── Interaction state (closure; the loop reads these) ──
  const camera = { pitch: initialCamera.pitch, yaw: initialCamera.yaw };
  ...
  const drawScene = () => {
    ...
    onHud({ pitch: camera.pitch, yaw: camera.yaw, zoomLabel: `x${focalLength.toFixed(2)}` });
  };
  ...
  // Attached by the consumer to its canvas element; state lives here so the
  // loop and the handlers can never disagree about a drag.
  ...
  window.addEventListener(liteModeEvent, onLiteToggle);
  ...
  return {
    ...
    destroy() {
      ...
      window.removeEventListener(liteModeEvent, onLiteToggle);
