        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 flex flex-col gap-6">
              ...
              <div className="space-y-4">
                <div className="bg-surface p-4 rounded border border-surface-variant">
                  <div className="flex justify-between mb-2">
                    <span className="font-working-title text-sm text-intel-cyan">PEZA Certification Status</span>
                    <span className="text-xs text-text-muted">Target: peza_status</span>
                  </div>
                  <select className="w-full bg-surface-alt border border-surface-variant p-2 rounded text-sm focus:outline-none focus:border-intel-cyan text-on-surface">
                ...
                <div className="bg-surface p-4 rounded border border-surface-variant">
                  <div className="flex justify-between mb-2">
                    <span className="font-working-title text-sm text-intel-cyan">Exact Floor Area (Sqm)</span>
                    <span className="text-xs text-text-muted">Target: floor_sqm</span>
                  </div>
                  <input type="number" className="w-full bg-surface-alt border border-surface-variant p-2 rounded text-sm focus:outline-none focus:border-intel-cyan text-on-surface" placeholder="e.g. 1200" />
                </div>
              </div>
            </div>
            ...
            <div className="card-atmosphere rounded-lg p-6">
              ...
              <textarea 
                className="w-full bg-surface border border-surface-variant rounded p-3 text-sm focus:outline-none focus:border-intel-cyan text-on-surface min-h-[100px]"
          ...
          <div className="lg:col-span-1">
            <div className="bg-surface border border-intel-cyan/30 rounded-lg p-6 shadow-lg sticky top-24">
              <div className="flex items-center gap-2 mb-4">
                <span className="text-2xl">💰</span>
                <h3 className="font-working-title text-lg text-on-surface">Bounty Reward</h3>
              </div>
              <div className="text-4xl font-display-md text-intel-cyan mb-2">{quest.bounty_connects || 5} <span className="text-lg">◈</span></div>
  ...
  return (
    <div className="flex flex-col gap-6 animate-[fadeIn_0.4s_ease]">
      ...
      <div className="flex border-b border-surface-variant overflow-x-auto">
        <button 
          className={`px-6 py-3 font-working-title text-sm border-b-2 transition whitespace-nowrap ${activeTab === 'audits' ? 'border-intel-cyan text-intel-cyan' : 'border-transparent text-text-secondary hover:text-on-surface'}`}
          onClick={() => setActiveTab("audits")}
        >
          Audit Board
        </button>
        <button 
          className={`px-6 py-3 font-working-title text-sm border-b-2 transition whitespace-nowrap ${activeTab === 'credentials' ? 'border-intel-cyan text-intel-cyan' : 'border-transparent text-text-secondary hover:text-on-surface'}`}
          ...
          {quests.map(quest => (
            ...
            >
              <div className="md:col-span-5">
                <h3 className="font-working-title text-base text-on-surface group-hover:text-intel-cyan transition">{quest.title}</h3>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 animate-[fadeIn_0.2s_ease]">
              ...
              <div>
                ...
                <div className="mb-4">
                  ...
                  <select 
                    ...
                    className="w-full bg-surface-alt border border-surface-variant p-3 rounded text-sm focus:outline-none focus:border-intel-cyan text-on-surface"
                  >
                  ...
                  <div className="mb-4">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className="w-full bg-surface-alt border border-surface-variant rounded p-3 pl-8 text-sm focus:outline-none focus:border-intel-cyan text-intel-cyan font-data-tabular font-bold" 
                      />
                      <span className="absolute left-3 top-3 text-intel-cyan">◈</span>
                    </div>
                  </div>
          ...
          <div className="xl:col-span-5 flex flex-col gap-4">
            <h3 className="font-working-title text-xl text-on-surface flex items-center gap-2">
              Investigator Dossier <span className="flex h-2 w-2 relative"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-intel-cyan opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-intel-cyan"></span></span>
            </h3>
            
            <div className="card-atmosphere rounded-lg p-6 relative overflow-hidden shadow-2xl sticky top-24 font-mono">
              <div className="absolute top-0 right-0 w-48 h-48 bg-intel-cyan opacity-[0.03] blur-[50px] rounded-full pointer-events-none"></div>
              ...
              <div className="flex items-start justify-between mb-6 relative z-10">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-surface-alt border border-surface-variant rounded flex items-center justify-center text-2xl relative">
                    📋
                    {acceptingAudits && (
                      <div className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-green-500 border-2 border-background rounded-full" title="Accepting Audits"></div>
                    )}
                  </div>
                  <div>
              ...
              <div className="flex flex-col gap-2 mb-6 border-y border-surface-variant py-4">
                ...
                {certLRA && (
                  <div className="flex items-center gap-2 text-sm text-on-surface">
                    <span className="text-intel-cyan">✓</span> LRA Direct Access (Title/Lien Tracing)
                  </div>
                )}
                {certLGU && (
                  <div className="flex items-center gap-2 text-sm text-on-surface">
                    <span className="text-intel-cyan">✓</span> LGU Zoning & Compliance Expediter
                  </div>
                )}
