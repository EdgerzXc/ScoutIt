...
export default function IntelError({ error, reset }) {
  useEffect(() => {
    reportError({
      kind: "crash",
      message: error?.message,
      stack: error?.stack,
      context: { digest: error?.digest, boundary: "app/intel/error.js" },
    }).catch(() => {});
  }, [error]);
    ...
    <div className="min-h-[70vh] flex items-center justify-center px-6 py-16 bg-[#0d0d0d]">
      <div className="max-w-md w-full text-center p-8 rounded-2xl bg-[#141414] border border-white/10 shadow-2xl" role="alert">
        ...
        <h2 className="font-serif text-2xl text-white mb-3">
          Unable to display intel briefing
        </h2>
        <p className="text-white/60 text-sm leading-relaxed mb-6">
          {message || "The market intelligence article could not be rendered."}
        </p>
